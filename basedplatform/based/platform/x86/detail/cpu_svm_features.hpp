...
namespace based::platform::x86::detail {
    ...
    enum class cpu_svm_feature_mask : std::uint32_t {
        ...
        enhanced_shutdown_intercept = 1u << std::to_underlying(cpu_svm_feature::enhanced_shutdown_intercept)
    }; // enum class cpu_svm_feature_mask : std::uint32_t

    [[nodiscard]]
    inline cpu_svm_feature_mask supported_cpu_svm_features(const std::uint32_t max_leaf) noexcept {
        auto result = cpu_svm_feature_mask{};

        if (max_leaf >= 0x8000000A) {
            using enum cpu_svm_feature;
            const auto leaf_0x8000000a = cpu_id(0x8000000A, 0x00);

            // edx register
            if (leaf_0x8000000a.edx_bit( 0u)) result |= np;
            if (leaf_0x8000000a.edx_bit( 1u)) result |= lbr_virt;
            if (leaf_0x8000000a.edx_bit( 2u)) result |= svml;
            if (leaf_0x8000000a.edx_bit( 3u)) result |= nrips;
            if (leaf_0x8000000a.edx_bit( 4u)) result |= tsc_rate_msr;
            if (leaf_0x8000000a.edx_bit( 5u)) result |= vmcb_clean;
            if (leaf_0x8000000a.edx_bit( 6u)) result |= flush_by_asid;
            if (leaf_0x8000000a.edx_bit( 7u)) result |= decode_assist;
            if (leaf_0x8000000a.edx_bit( 8u)) result |= pmc_virt;
            if (leaf_0x8000000a.edx_bit(10u)) result |= pause_filter;
            if (leaf_0x8000000a.edx_bit(12u)) result |= pause_filter_threshold;
            if (leaf_0x8000000a.edx_bit(13u)) result |= avic;
            if (leaf_0x8000000a.edx_bit(15u)) result |= vmsave_virt;
            if (leaf_0x8000000a.edx_bit(16u)) result |= vgif;
            if (leaf_0x8000000a.edx_bit(17u)) result |= gmet;
            if (leaf_0x8000000a.edx_bit(18u)) result |= x2avic;
            if (leaf_0x8000000a.edx_bit(19u)) result |= sss_check;
            if (leaf_0x8000000a.edx_bit(20u)) result |= spec_ctrl;
            if (leaf_0x8000000a.edx_bit(21u)) result |= rogpt;
            if (leaf_0x8000000a.edx_bit(23u)) result |= host_mce_override;
            if (leaf_0x8000000a.edx_bit(24u)) result |= tlbi_ctl;
            if (leaf_0x8000000a.edx_bit(25u)) result |= vnmi;
            if (leaf_0x8000000a.edx_bit(26u)) result |= ibs_virt;
            if (leaf_0x8000000a.edx_bit(27u)) result |= ext_lvt_offset_fault_chg;
            if (leaf_0x8000000a.edx_bit(28u)) result |= vmcb_addr_chk_chg;
            if (leaf_0x8000000a.edx_bit(29u)) result |= bus_lock_threshold;
            if (leaf_0x8000000a.edx_bit(30u)) result |= idle_hlt_intercept;
            if (leaf_0x8000000a.edx_bit(31u)) result |= enhanced_shutdown_intercept;
        }
