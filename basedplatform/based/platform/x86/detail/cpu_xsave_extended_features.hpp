...
namespace based::platform::x86::detail {
    ...
    [[nodiscard]]
    inline cpu_combined_max_save_save_area_mask supported_cpu_combined_max_xsave_save_area(const std::uint32_t max_leaf) noexcept {
    ...
    [[nodiscard]]
    inline cpu_state_component_mask supported_cpu_ia32_xss_state_components(const std::uint32_t max_leaf) noexcept {
        ...
        if (max_leaf >= 0x0D) {
            ...
            // ecx register
            if (leaf_0x0d.ecx_bit( 8u)) result |= processor_trace_state;
            if (leaf_0x0d.ecx_bit(10u)) result |= pasid;
            if (leaf_0x0d.ecx_bit(11u)) result |= cet_u_state;
            if (leaf_0x0d.ecx_bit(12u)) result |= cet_s_state;
            if (leaf_0x0d.ecx_bit(13u)) result |= hdc;
            if (leaf_0x0d.ecx_bit(14u)) result |= uintr;
            if (leaf_0x0d.ecx_bit(15u)) result |= lbr;
            if (leaf_0x0d.ecx_bit(16u)) result |= hwp;
        }
