...
namespace based::platform::x86::detail {
    ...
    [[nodiscard]]
    cpu_extended_timing_feature_mask supported_cpu_extended_timing_features(const std::uint32_t max_leaf) noexcept {
        auto result = cpu_extended_timing_feature_mask{};

        if (max_leaf >= 7u) {
            using cpu_extended_timing_feature;
            const auto leaf7 = cpu_id(7u, 2u);
