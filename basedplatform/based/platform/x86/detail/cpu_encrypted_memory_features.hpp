...
namespace based::platform::x86::detail {
    ...
    enum class cpu_encrypted_memory_feature_mask : std::uint32_t {
        ...
        host_64_bit            = 1u << std::to_underlying(cpu_encrypted_memory_feature::host_64_bit), // 64BitHost
    ...
    cpu_encrypted_memory_feature_mask supported_cpu_encrypted_memory_features(const std::uint32_t max_leaf) noexcept {
        ...
        if (max_leaf >= 0x8000001F) {
            using cpu_encrypted_memory_feature;
            const auto leaf8 = cpu_id(0x8000001F, 0u);

            // eax register
            if (leaf8.eax_bit( 0u)) result |= sme;
            if (leaf8.eax_bit( 1u)) result |= sev;
            if (leaf8.eax_bit( 2u)) result |= page_flush_msr;
            if (leaf8.eax_bit( 3u)) result |= sev_es;
            if (leaf8.eax_bit( 4u)) result |= sev_snp;
            if (leaf8.eax_bit( 5u)) result |= vmpl;
            if (leaf8.eax_bit( 6u)) result |= rmpquery;
            if (leaf8.eax_bit( 7u)) result |= vmpl_sss;
            if (leaf8.eax_bit( 8u)) result |= secure_tsc;
            if (leaf8.eax_bit( 9u)) result |= tsc_aux_virtualization;
            if (leaf8.eax_bit(10u)) result |= hw_enf_cache_coh;
            if (leaf8.eax_bit(11u)) result |= host_64_bit;
            if (leaf8.eax_bit(12u)) result |= restricted_injection;
            if (leaf8.eax_bit(13u)) result |= alternate_injection;
            if (leaf8.eax_bit(14u)) result |= debug_virt;
            if (leaf8.eax_bit(15u)) result |= prevent_host_ibs;
            if (leaf8.eax_bit(16u)) result |= vte;
            if (leaf8.eax_bit(17u)) result |= vmgexit_parameter;
            if (leaf8.eax_bit(18u)) result |= virtual_tom_msr;
            if (leaf8.eax_bit(19u)) result |= ibs_virt_guest_ctl;
            if (leaf8.eax_bit(20u)) result |= pmc_virt_guest_ctl;
            if (leaf8.eax_bit(21u)) result |= rmpread;
            if (leaf8.eax_bit(24u)) result |= vmsa_reg_prot;
            if (leaf8.eax_bit(25u)) result |= smt_protection;
            if (leaf8.eax_bit(26u)) result |= secure_avic;
            if (leaf8.eax_bit(27u)) result |= allowed_sev_features;
            if (leaf8.eax_bit(28u)) result |= svsm_comm_page_msr;
            if (leaf8.eax_bit(29u)) result |= nested_virt_snp_msr;
            if (leaf8.eax_bit(30u)) result |= hv_in_use_wr_allowed;
            if (leaf8.eax_bit(31u)) result |= ibpb_on_entry;
        }
