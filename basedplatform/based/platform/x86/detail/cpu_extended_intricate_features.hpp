...
namespace based::platform::x86::detail {
    ...
    [[nodiscard]]
    cpu_extended_intrcate_feature_mask supported_extended_intricate_features(const std::uint32_t max_leaf) noexcept {
        auto result = cpu_extended_intricate_feature_mask{};

        if (max_leaf >= 7u) {
            using cpu_extended_intricate_feature;
            const auto leaf7 = cpu_id(7u, 0u);
