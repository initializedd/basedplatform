...
namespace based::platform::x86::detail {
    ...
    enum class cpu_power_management_feature_mask : std::uint32_t {
        ...
        steps100mhz             = 1u << std::to_underlying(cpu_power_management_feature::steps100mhz),
    ...
    inline cpu_power_management_feature_mask supported_cpu_power_management_features(const std::uint32_t max_leaf) noexcept {
        ...
        if (max_leaf >= 0x80000007) {
            using enum cpu_power_management_feature;
            const auto leaf_0x80000007 = cpu_id(0x80000007, 0x00);

            // edx register
            if (leaf_0x80000007.edx_bit( 0u)) result |= ts;
            if (leaf_0x80000007.edx_bit( 1u)) result |= fid;
            if (leaf_0x80000007.edx_bit( 2u)) result |= vid;
            if (leaf_0x80000007.edx_bit( 3u)) result |= ttp;
            if (leaf_0x80000007.edx_bit( 4u)) result |= tm;
            if (leaf_0x80000007.edx_bit( 5u)) result |= stc;
            if (leaf_0x80000007.edx_bit( 6u)) result |= steps100mhz;
            if (leaf_0x80000007.edx_bit( 7u)) result |= hw_pstate;
            if (leaf_0x80000007.edx_bit( 8u)) result |= tsc_invariant;
            if (leaf_0x80000007.edx_bit( 9u)) result |= cpb;
            if (leaf_0x80000007.edx_bit(10u)) result |= eff_freq_ro;
            if (leaf_0x80000007.edx_bit(11u)) result |= proc_feedback_interface;
            if (leaf_0x80000007.edx_bit(12u)) result |= proc_power_reporting;
            if (leaf_0x80000007.edx_bit(13u)) result |= connected_standby;
            if (leaf_0x80000007.edx_bit(14u)) result |= rapl;
            if (leaf_0x80000007.edx_bit(15u)) result |= fast_cppc;
        }
