...
namespace based::platform::x86::detail {
    ...
    inline std::bitset<256> supported_cpu_descriptors(const std::uint32_t max_leaf) noexcept {
        ...
        if (max_leaf >= 2u) {
            const auto leaf2 = cpu_id(2u, 0u);

            result.set((leaf2.eax >>  0u) & 0xff); // eax byte 0
            result.set((leaf2.eax >>  8u) & 0xff); // eax byte 1
            result.set((leaf2.eax >> 16u) & 0xff); // eax byte 2
            result.set((leaf2.eax >> 24u) & 0xff); // eax byte 3

            result.set((leaf2.ebx >>  0u) & 0xff); // ebx byte 0
            result.set((leaf2.ebx >>  8u) & 0xff); // ebx byte 1
            result.set((leaf2.ebx >> 16u) & 0xff); // ebx byte 2
            result.set((leaf2.ebx >> 24u) & 0xff); // ebx byte 3

            result.set((leaf2.ecx >>  0u) & 0xff); // ecx byte 0
            result.set((leaf2.ecx >>  8u) & 0xff); // ecx byte 1
            result.set((leaf2.ecx >> 16u) & 0xff); // ecx byte 2
            result.set((leaf2.ecx >> 24u) & 0xff); // ecx byte 3

            result.set((leaf2.edx >>  0u) & 0xff); // edx byte 0
            result.set((leaf2.edx >>  8u) & 0xff); // edx byte 1
            result.set((leaf2.edx >> 16u) & 0xff); // edx byte 2
            result.set((leaf2.edx >> 24u) & 0xff); // edx byte 3
        }
